...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (grounded)
        {
            rb.linearDamping = groundDrag;  // Apply ground drag if grounded
        }
        else
        {
            rb.linearDamping = 0;  // No drag if not grounded
        }
    }
    ...
    {
        return rb.linearVelocity;  // Return current velocity of the Rigidbody
    }
    ...
    {
        Vector3 flatVel = new Vector3(rb.linearVelocity.x, 0f, rb.linearVelocity.z);  // Remove vertical component from velocity

        // Limiting velocity to moveSpeed
        if (flatVel.magnitude > moveSpeed)
        {
            Vector3 limitedVel = flatVel.normalized * moveSpeed;  // Limit velocity magnitude
            rb.linearVelocity = new Vector3(limitedVel.x, rb.linearVelocity.y, limitedVel.z);  // Apply limited velocity
        }
    }
